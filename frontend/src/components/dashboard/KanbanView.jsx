...
// TaskCard Component
const TaskCard = ({ task, projectTeam, onDragStart, onEdit, onDelete, isMobile }) => {
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-lg p-3 sm:p-4 shadow-sm border border-gray-200 cursor-move hover:shadow-md transition-shadow"
    >
      {/* Task Header */}
      <div className="flex items-start justify-between mb-2">
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          ...
          <button
            ...
            className="p-1 text-gray-400 hover:text-red-600 rounded touch-target"
          >
        ...
        <div className="flex flex-wrap gap-1 mb-3">
          ...
          {task.tags.length > (isMobile ? 1 : 2) && (
            <span className="text-xs text-gray-500">
              +{task.tags.length - (isMobile ? 1 : 2)} more
            </span>
          )}
        </div>
      )}
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          {task.dueDate && (
            <div className={`flex items-center gap-1 text-xs ${
              ...
              <span className="sm:hidden">{new Date(task.dueDate).getDate()}</span>
            </div>
          )}
          ...
          {task.comments && task.comments.length > 0 && (
            <div className="flex items-center gap-1 text-xs text-gray-500">
              ...
              <span className="hidden sm:inline">{task.comments.length}</span>
            </div>
          )}
        </div>
            ...
            {typeof task.assignee === 'object' ? (
              ...
              (() => {
                ...
                return teamMember ? (
                  ...
                  <div className="w-5 h-5 sm:w-6 sm:h-6 bg-gray-400 rounded-full flex items-center justify-center text-white text-xs font-medium">
                    ?
                  </div>
                )
              })()
...
// EmptyColumnState Component
const EmptyColumnState = ({ column, onAddTask }) => (
  <div className="text-center py-6 sm:py-8 text-gray-400">
    ...
    <p className="text-xs sm:text-sm">No tasks in {column.title.toLowerCase()}</p>
    {column.id === 'todo' && (
      <button
        onClick={onAddTask}
        className="text-blue-600 hover:text-blue-700 text-xs sm:text-sm mt-1 touch-target"
      >
        Add your first task
      </button>
    )}
    ...
    <div className="h-full flex flex-col p-responsive">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4 sm:mb-6">
        <div className="flex-1 min-w-0">
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-2">Kanban Board</h2>
          <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-xs sm:text-sm text-gray-600">
            <span className="truncate">
              Project: {currentProject?.name || 'Loading...'}
            </span>
            <span>Tasks: {projectTasks.length}</span>
            <span>Team: {projectTeam.length} members</span>
            {loading.tasks && (
        ...
        <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors disabled:opacity-50 touch-target"
            title="Refresh tasks"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors touch-target flex-1 sm:flex-none justify-center"
          >
        ...
        <div className="lg:hidden">
          <div className="flex gap-4 overflow-x-auto pb-4 h-full" style={{ minWidth: 'max-content' }}>
            {columns.map((column) => (
              <div
                key={column.id}
                className={`${column.color} rounded-lg p-4 flex flex-col min-w-[280px] max-w-[280px] h-full`}
              ...
              >
                ...
                <div className="flex-1 space-y-3 overflow-y-auto">
                  ...
                  {getTasksByStatus(column.id).length === 0 && (
                    <EmptyColumnState column={column} onAddTask={() => setShowNewTaskModal(true)} />
                  )}
                </div>
              </div>
            ))}
        ...
        {showNewTaskModal && (
          ...
          >
            <motion.div
              ...
              className="bg-white rounded-lg w-full max-w-md max-h-[90vh] overflow-y-auto modal-scroll"
              ...
              <form onSubmit={handleCreateTask} className="p-4 sm:p-6 space-y-4">
                ...
                <div className="flex flex-col sm:flex-row justify-end gap-3 pt-4 border-t border-gray-200">
                  <button
                    type="button"
                    onClick={() => setShowNewTaskModal(false)}
                    className="px-4 py-2.5 text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors touch-target order-2 sm:order-1"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="px-4 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors touch-target order-1 sm:order-2"
                  >
                    Create Task
                  </button>
                </div>
